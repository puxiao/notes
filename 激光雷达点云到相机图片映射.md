...
**下面关于 "点云3D空间中某个点映射到相机2D图片中" 的内容是之前的映射算法，会存在以下 2 个问题：**
...
所以，下面的算法代码在实际中会存在一些异常情况。

例如：假定立方体 8 个顶点有一些处于相机前方，有一些出于相机后方，此时依然要准确得到映射就结果，之前的算法就无法正确处理。
...
* 之前是将 3D 点 applyMatrix4() 相机内参和外出 综合作用下的矩阵 cameraMatrix
